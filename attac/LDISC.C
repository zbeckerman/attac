...
void ldisc_send(void *handle, char *buf, int len, int interactive)
{
	...
	ATTACScripting_WaitForSendData(0);
	
	if (len>=4)
	{
		strcpy (key, "");

		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==51)
		{
			strcpy (key, "F3");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==52)
		{
			strcpy (key, "F4");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==53)
		{
			strcpy (key, "F5");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==55)
		{
			strcpy (key, "F6");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==56)
		{
			strcpy (key, "F7");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==49 && buf[3]==57)
		{
			strcpy (key, "F8");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==48)
		{
			strcpy (key, "F9");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==49)
		{
			strcpy (key, "F10");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==51)
		{
			strcpy (key, "F11");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==52)
		{
			strcpy (key, "F12");
		}

		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==53)
		{
			strcpy (key, "Shift F3");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==54)
		{
			strcpy (key, "Shift F4");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==56)
		{
			strcpy (key, "Shift F5");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==50 && buf[3]==57)
		{
			strcpy (key, "Shift F6");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==51 && buf[3]==49)
		{
			strcpy (key, "Shift F7");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==51 && buf[3]==50)
		{
			strcpy (key, "Shift F8");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==51 && buf[3]==51)
		{
			strcpy (key, "Shift F9");
		}
		if (buf[0]==27 && buf[1]==91 && buf[2]==51 && buf[3]==52)
		{
			strcpy (key, "Shift F10");
		}
		...
		if (strcmp (key, "") != 0)
		{
			for (i=0; i<macroCount; i++)
				{
					...
					{
						t = (char *) strdup(theMacros[i].data);
						if (Instr(1, (char *) strlwr(t), "rexx:") >0)
						{
							tmp = right (theMacros[i].data, strlen (theMacros[i].data)-5);
							if (tmp[0]==' ')
							{
								free(tmp);
								tmp = right (theMacros[i].data, strlen (theMacros[i].data)-6);			
							}
							REXX_Start(tmp, 0, 0, rexxArgs);
							free (tmp);
						}
						else
						{
							if (isRegistered==0)
							{
								if (strlen(theMacros[i].data) <=20)
									attac_send(theMacros[i].data);
								else
									attacErrorMessage ("Unregistered versions of attac can only send macros that are up to 20 characters in length");
							}
							else
								attac_send(theMacros[i].data);
						}
			...
			ATTACScripting_ReleaseSendData();
			return;
		}
	}

	if (REXX_LockKeyboard==1 && buf[0]!=27)
	{
		ATTACScripting_ReleaseSendData();
		return;
	}
	...
	if (buf != NULL && buf[0] != 0)
	{
		...
		if (commandPrompt==6)
		{
			/* if there are no keys defined... let everything thru */
			if (header.validStardockKeys[0]!='\x0')
				validKey=0;

			/* loop thru all valid keys and match them */
			for (i=0; i<(int) strlen(header.validStardockKeys) && validKey==0; i++)
			{
				if (tolower(buf[0])==tolower(header.validStardockKeys[i]))
				{
					commandPrompt=7;  /* set value as not at main SD prompt */
					validKey=1;   /* say key is valid */
				}
			}
		}
	}

	//break out if we are at SD and you hit an invalid key
	if (validKey==0 && REXX_RunningScript==0 && runningTWXScript==0)
	{
		attacErrorMessage("Invalid Stardock Key");
		ATTACScripting_ReleaseSendData();
		return;
	}

	//pop up windows
	if (buf != NULL && buf[0] == '`' && isFedCommVisible==0 && isPrivateCommVisible==0 && header.popupComms==1)
	{
		showfedcommform(globalHWnd);
		ATTACScripting_ReleaseSendData();
		return;
	}
	...
	if (buf != NULL && buf[0] == '\'' && isFedCommVisible==0 && isPrivateCommVisible==0 && header.popupComms==1)
	{
		showprivatecommform(globalHWnd);
		ATTACScripting_ReleaseSendData();
		return;
	}
	...
	{
		if (recordFastMacro < FASTMACROLEN)
		{
			if ((int)buf[0]==13)
			{
				fastMacroString[recordFastMacro++]='^';
				fastMacroString[recordFastMacro++]='M';
			}
			else if ((int)buf[0]==10)
			{
				fastMacroString[recordFastMacro++]='^';
				fastMacroString[recordFastMacro++]='J';
			}
			else
				fastMacroString[recordFastMacro++]=buf[0];
			fastMacroString[recordFastMacro]=0;
		}
		else
		{
			sprintf (buffer, "Fast macro STOPPED recording since it exceeeds it size of %d characters", FASTMACROLEN);
			attacErrorMessage(buffer);
			SendMessage(globalHWnd, WM_COMMAND, (WPARAM) IDM_RECORDFASTMACRO, (LPARAM) 0);
		}
	}
