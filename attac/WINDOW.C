...
#include "storage.h"
#ifdef ATTAC_CODE
	#include "attac_src/attacUtils.h"
	#include "attac_src/attacGUI.h"
	#include "attac_src/win_res.h"
	#include "attac_src/rexx.h"
	#include "attac_src/info.h"
	#include "attac_src/parsedatathread.h"
	#include "attac_src/statusthread.h"
	#include "attac_src/autoupdate.h"
	#include "attac_src/remoteConnection.h"
	#include "attac_src/twx.h"
	#include "attac_src/cim.h"
	#include "attac_src/xml.h"
	#include "attac_src/twxSync.h"
...
int WINAPI WinMain(HINSTANCE inst, HINSTANCE prev, LPSTR cmdline, int show)
{
	...
	int returnVal=0;
	HANDLE hmenu;
//	HANDLE hTool, hStatus;
	int counter;
	HACCEL haccel;

	GetCurrentDirectory (MAXAPPDIRLEN, appDir);
	strcat (appDir, "\\");

	strcpy (scriptDir, appDir);
	strcpy (exeDir, appDir);

	strcat (scriptDir, "scripts\\");
	strcat (appDir, "datafiles\\");
#endif
...
#ifdef ATTAC_CODE
		wndclass.lpszMenuName = (LPCSTR)IDC_ATTAC_MENU;
#endif
	...
	for (j = 0; j < lenof(popup_menus); j++) {
	    ...
	    sfree(str);
#ifdef ATTAC_CODE
		AppendMenu(m, MF_ENABLED, IDM_REXX, "&Run REXX");
#endif
...
#ifdef ATTAC_CODE
	Send_Data_Mutex = CreateMutex(NULL, 0, NULL);
	RemoveCRsMutex = CreateMutex(NULL, 0, NULL);
	PFreeMutex = CreateMutex(NULL, 0, NULL);
	CleanLineMutex = CreateMutex(NULL, 0, NULL);
	UnCommaMutex = CreateMutex(NULL, 0, NULL);
	SyncDataMutex = CreateMutex(NULL, 0, NULL);
	MessageMutex = CreateMutex(NULL, 0, NULL);
	StatusBarMutex = CreateMutex(NULL, 0, NULL);
	tempCurrentSectorMutex = CreateMutex(NULL, 0, NULL);
	Data_Mutex = CreateMutex(NULL, 0, NULL);
	MemoryData_Mutex = CreateMutex(NULL, 0, NULL);
	statusThreadsMutex = CreateMutex(NULL, 0, NULL);
	parseThreadsMutex = CreateMutex(NULL, 0, NULL);
	buildTWXDataMutex = CreateMutex(NULL, 0, NULL);

	showmessagecenterform(hwnd);
	showinfocenterform(hwnd);

	if (showmessagecenter!=NULL) {
		PostMessage(showmessagecenter, WM_PARENTMOVED, 0, 0);
		PostMessage(showmessagecenter, WM_PARENTSIZED, 0, 0);
	}
	if (showinfocenter !=NULL) {
		PostMessage(showinfocenter, WM_PARENTMOVED, 0, 0);
		PostMessage(showinfocenter, WM_PARENTSIZED, 0, 0);
	}
	// Fill it with stuff
	update_status_bar();
	...
	//load the trader titles
	loadTraderTitles();
	ZocReceiveBuf=NULL;


	if(_beginthread(ParseDataThreadProc, 0, NULL )==-1)
	{
		printf("Failed to create thread");
		return 1;
	}

	if(_beginthread(StatusThreadProc, 0, NULL )==-1)
	{
		printf("Failed to create thread");
		return 1;
	}
	if(_beginthread(WriteMemoryThreadProc, 0, NULL )==-1)
	{
		printf("Failed to create thread");
		return 1;
	}
/*
	if(_beginthread(AutoUpdateThreadProc, 0, NULL )==-1)
	{
		printf("Failed to create thread");
		return 1;
	}
*/	
	for (counter=0; counter < TOTAL_CONNECTIONS; counter++)
		theClients[counter].sock=0;

	theMacros = (MACRO_STRUCT *) malloc(MAX_MACROS * sizeof(MACRO_STRUCT));
	...
	{
		char *prog = strdup ("ATTAC");
		char *beta = strdup (betaVersion);
		char *user = strdup (theUserId);

		authorizeProgramExternal(&authorizeCallback, prog, user, atoi(releaseNumber), atoi(majorNumber), atoi(minorNumber), beta);
	...
	if (helperActive==1)
	{
		CheckMenuItem(hmenu, IDM_HELPERACTIVE, MF_CHECKED);
	}
	
	haccel = LoadAccelerators(inst, MAKEINTRESOURCE(IDC_ATTAC_ACCELS));
	if (haccel == NULL)
		attacErrorMessage("Error Loading Accelerators");

	recordFastMacro=-1;

	//start the twxSyncTimerProc
	//pulse the event every 5 minutes
	SetTimer(globalHWnd, 2, 210000, twxSyncTimerProc);
	...
	while (msg.message != WM_QUIT) {
		...
		// i use this to make sure that my rexxask and rexxaskp and the logbox get the control correctly
		if (!(IsWindow(logbox) && IsDialogMessage(logbox, &msg)) &&
			!(IsWindow(showcolonization) && IsDialogMessage(showcolonization, &msg)) &&
			!(IsWindow(showmap) && IsDialogMessage(showmap, &msg)) &&
			!(IsWindow(showbuydump) && IsDialogMessage(showbuydump, &msg)) &&
			!(IsWindow(showpairport) && IsDialogMessage(showpairport, &msg)) &&
			!(IsWindow(showcimoptions) && IsDialogMessage(showcimoptions, &msg)) &&
			!(IsWindow(showregistration) && IsDialogMessage(showregistration, &msg)) &&
			!(IsWindow(showquasarcalculator) && IsDialogMessage(showquasarcalculator, &msg)) &&
			!(IsWindow(showbubblefinder) && IsDialogMessage(showbubblefinder, &msg)) &&
			!(IsWindow(showbuyplanetshields) && IsDialogMessage(showbuyplanetshields, &msg)) &&
			!(IsWindow(showstealmove) && IsDialogMessage(showstealmove, &msg)) &&
			!(IsWindow(showstealtransport) && IsDialogMessage(showstealtransport, &msg)) &&
			!(IsWindow(showworldtrade) && IsDialogMessage(showworldtrade, &msg)) &&
			!(IsWindow(showworldssm) && IsDialogMessage(showworldssm, &msg)) &&
			!(IsWindow(showztm) && IsDialogMessage(showztm, &msg)) &&
			!(IsWindow(showmegajettison) && IsDialogMessage(showmegajettison, &msg)) &&
			!(IsWindow(showpp) && IsDialogMessage(showpp, &msg)) &&
			!(IsWindow(showevilpp) && IsDialogMessage(showevilpp, &msg)) &&
			!(IsWindow(showbustlist) && IsDialogMessage(showbustlist, &msg)) &&
			!(IsWindow(showmaxports) && IsDialogMessage(showmaxports, &msg)) &&
			!(IsWindow(showmover) && IsDialogMessage(showmover, &msg)) &&
			!(IsWindow(showsectorinfo) && IsDialogMessage(showsectorinfo, &msg)) &&
			!(IsWindow(shownearestfig) && IsDialogMessage(shownearestfig, &msg)) &&
			!(IsWindow(showfedcomm) && IsDialogMessage(showfedcomm, &msg)) &&
			!(IsWindow(showprivatecomm) && IsDialogMessage(showprivatecomm, &msg)) &&
			!(IsWindow(showautoexplorer) && IsDialogMessage(showautoexplorer, &msg)) &&
			!(IsWindow(showmacro) && IsDialogMessage(showmacro, &msg)) &&
			!(IsWindow(showmacroedit) && IsDialogMessage(showmacroedit, &msg)) &&
			!(IsWindow(showplanetbust) && IsDialogMessage(showplanetbust, &msg)) &&
			!(IsWindow(showaddbust) && IsDialogMessage(showaddbust, &msg)) &&
			!(IsWindow(rexxask) && IsDialogMessage(rexxask, &msg)) &&
			!(IsWindow(showsellplanettoport) && IsDialogMessage(showsellplanettoport, &msg)) &&
			!(IsWindow(showtwxsync) && IsDialogMessage(showtwxsync, &msg)) 
			)
...
void authorizeCallback (int isRegistrationValid, char *returnBuffer) {
	if (isRegistrationValid==INVALID_REGISTRATION) {
		validRegCode=0;
		isRegistered=0;
		validTWXSyncRegCode=0;
		attacErrorMessage(returnBuffer);
	}
	if (isRegistrationValid==INVALID_VERSION) {
		validRegCode=0;
		isRegistered=0;
		validTWXSyncRegCode=0;
		attacErrorMessage(returnBuffer);
				...
				WPARAM wParam, LPARAM lParam)
{
	...
	char tmpFileName[256], buffer[100], buffer2[150];
	char *tmpPtr;
	HANDLE hmenu=NULL;
	FILE *fp=NULL;
#endif
	...
	case IDM_RUN_REXX:
...
#endif
		of.hwndOwner = hwnd;
		of.lpstrFilter = "REXX Scripting Files\0*.REXX;*.ZRX\0Encrypted REXX Scripting Files\0*.ERX\0All Files\0*\0\0\0";
		...
		of.lpstrTitle = "Select REXX File";
		of.Flags = 0;
		if (GetOpenFileName(&of)) {
			//with erx, use encrypted
			if (Instr(1, filename, ".erx"))
				REXX_Start(filename, 1, 0, rexxArgs);
			else
				REXX_Start(filename, 0, 0, rexxArgs);
		}
 		break;
	case IDM_PORT_BUILDER:
		break;
	case IDM_CITADEL_BUILDER:
		break;
	case IDM_QUASAR_CALCULATOR:
		showquasarcalculatorform(hwnd);
		break;
	case IDM_SHIP_CAPTURE:
		break;
	case IDM_MOVE_TO_SECTOR:
		strcpy (fullFilePath, scriptDir);
		strcat (fullFilePath, "StandardMoveMe.erx");
		REXX_Start(fullFilePath, 1, 0, rexxArgs);
		break;
	case IDM_PHOTON_AND_MOVE:
		strcpy (fullFilePath, scriptDir);
		strcat (fullFilePath, "StandardPhotonAndMove.erx");
		REXX_Start(fullFilePath, 1, 0, rexxArgs);
		break;
	case IDM_USE_MINE_DISRUPTERS:
		strcpy (fullFilePath, scriptDir);
		strcat (fullFilePath, "StandardMineDisrupter.erx");
		REXX_Start(fullFilePath, 1, 0, rexxArgs);
		break;
	case IDM_ATTAC_ESCAPETOSECTOR:
		strcpy (fullFilePath, scriptDir);
		strcat (fullFilePath, "StandardEscapeSector.erx");
		sprintf (rexxArgs[0], "%ld", header.escapeSector);
		REXX_Start(fullFilePath, 1, 1, rexxArgs);
		break;
	case IDM_PAY_TOLLS:
		showpaytollsform(hwnd);
		break;
	case IDM_BLIND_TWARP_PROTECTION:
		hmenu = GetMenu(hwnd); 
		if (header.blindTwarpProtection==0)
		{
			header.blindTwarpProtection= 1;
			CheckMenuItem(hmenu, IDM_BLIND_TWARP_PROTECTION, MF_CHECKED);
		}
		else
		{
			header.blindTwarpProtection= 0;
			CheckMenuItem(hmenu, IDM_BLIND_TWARP_PROTECTION, MF_UNCHECKED);
		}
	
		writeMemory();
		break;
	case IDM_SELF_DESTRUCT_PROTECTION:
		hmenu = GetMenu(hwnd); 
		if (header.selfDestructProtection==0)
		{
			header.selfDestructProtection= 1;
			CheckMenuItem(hmenu, IDM_SELF_DESTRUCT_PROTECTION, MF_CHECKED);
		}
		else
		{
			header.selfDestructProtection= 0;
			CheckMenuItem(hmenu, IDM_SELF_DESTRUCT_PROTECTION, MF_UNCHECKED);
		}
	...
	case IDM_HELPERACTIVE:
		...
		{
			helperActive= 0;
			CheckMenuItem(hmenu, IDM_HELPERACTIVE, MF_UNCHECKED);
		}
		break;
	case IDM_EXPORTDBSECTORS:
		strcpy (fullFilePath, appDir);
		strcat (fullFilePath, "unexplored.txt");
		fp = fopen (fullFilePath,"w");
		if (fp==NULL)
		{
			sprintf (buffer, "ErrorCode: %d -- Cant open file %s", GetLastError(), fullFilePath);
		...
		else
		{
			...
			fclose (fp);
			sprintf (buffer, "%s exported successfully", fullFilePath);
			attacErrorMessage (buffer);
		}
		break;
	case IDM_MESSAGE_CENTER:
		showmessagecenterform(hwnd);
		PostMessage(showmessagecenter, WM_PARENTMOVED, wParam, lParam);
		PostMessage(showmessagecenter, WM_PARENTSIZED, wParam, lParam);
		break;
	case IDM_STATUS_CENTER:
		showinfocenterform(hwnd);
		update_status_bar();
		PostMessage(showinfocenter, WM_PARENTMOVED, wParam, lParam);
		PostMessage(showinfocenter, WM_PARENTSIZED, wParam, lParam);
		break;
	case IDM_MACROS:
		showmacroform(hwnd);
		break;
	case IDM_OPTIONS:
		...
		break;
	case IDM_DEAD_END:
		showdeadendform(hwnd);
		break;
	case IDM_BUBBLE:
		showbubblefinderform(hwnd);
		break;
	case IDM_PLANET:
		showplanetform(hwnd);
		break;
	case IDM_AVOID:
		show_listbox_data(hwnd, 3);
		break;
	case IDM_FIGHTER:
		showfighterform(hwnd);
		break;
	case IDM_BLOCKED_PORTS:
		showblockedportform(hwnd);
		break;
	case IDM_NEARESTFIGHTERS:
		shownearestfigform(hwnd);
		break;
	case IDM_MAP:
		showmapform(hwnd);
		break;
	case IDC_DEBUG:
		...
		break;
	case IDM_SECTORINFO:
		showsectorinfoform(hwnd);
		break;
	case IDM_MAX_PORT:
		showmaxportsform(hwnd);
		break;
	case IDM_CONSTRUCTION_PORT:
		show_listbox_data(hwnd, 1);
		break;
	case IDM_CLASS0_PORT:
		show_listbox_data(hwnd, 2);
		break;
	case IDM_ONEWAY_SECTORS:
		show_listbox_data(hwnd, 4);
		break;
	case IDM_COLONIZE:
		showcolonizationform(hwnd);
 		break;
	case IDM_BUYDUMP:
		showbuydumpform(hwnd);
		break;
	case IDM_SELLPLANETTOPORT:
		showsellplanettoportform(hwnd);
		break;
	case IDM_PLANETBUST:
		showplanetbustform(hwnd);
		break;
	case IDM_BUYPLANETSHIELDS:
		showbuyplanetshieldsform(hwnd);
		break;
	case IDM_MOVECOLONISTS:
		showmoverform(hwnd, 1);
		break;
	case IDM_MOVERESOURCES:
		showmoverform(hwnd, 2);
		break;
	case IDM_PAIRPORT_TRADING:
		showpairportform(hwnd);
		break;
	case IDM_MEGAJETTISON:
		showmegajettisonform(hwnd);
		break;
	case IDM_CLASS1:
		showporttypeform(hwnd, 1);
		break;
	case IDM_CLASS2:
		showporttypeform(hwnd, 2);
		break;
	case IDM_CLASS3:
		showporttypeform(hwnd, 3);
		break;
	case IDM_CLASS4:
		showporttypeform(hwnd, 4);
		break;
	case IDM_CLASS5:
		showporttypeform(hwnd, 5);
		break;
	case IDM_CLASS6:
		showporttypeform(hwnd, 6);
		break;
	case IDM_CLASS7:
		showporttypeform(hwnd, 7);
		break;
	case IDM_CLASS8:
		showporttypeform(hwnd, 8);
		break;
	case IDM_PAIRPORTS:
		show_pair_ports(hwnd);
		break;
	case IDM_WORLDTRADE:
		showworldtradeform(hwnd);
		break;
	case IDM_SSM:
		showstealmoveform(hwnd);
		break;
	case IDM_SST:
		showstealtransportform(hwnd);
		break;
	case IDM_BUSTLIST:
		showbustlistform(hwnd);
		break;
	case IDM_EVILPAIRS:
		show_evil_pair_ports(hwnd);
		break;
	case IDM_WORLDSSM :
		showworldssmform(hwnd);
		break;
	case IDM_CIM:
		showcimoptionsform(hwnd);
		break;
	case IDM_ZTM:
		showztmform(hwnd);
		break;
	case IDM_AUTOEXPLORE:
		showautoexplorerform(hwnd);
		break;
	case IDM_ETHEREXPLORE:
		showetherexploreform(hwnd);
		break;
	case IDM_ETHEREXPLORE_LOG:
		showetherexplorelogform(hwnd);
		break;
	case IDM_IMPORTTWX:
		memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
		...
		of.lpstrFileTitle = NULL;
		of.lpstrInitialDir = NULL;
		of.lpstrTitle = "Import TWX v1 File";
		of.Flags = 0;
		if (GetOpenFileName(&of)) 
		{
			...
			if (strlen(filename)<5)
				strcat(tmpFileName, ".twx");
			else if (strstr(filename, ".twx")==NULL)
				strcat(tmpFileName, ".twx");

			returnVal = import_twx(tmpFileName);

			//if it was a good import
			if (returnVal==1)
			{
				// write this new data to memory
				writeMemory();

				sprintf (buffer, "TWX v1 file %s imported", tmpFileName);
				attacInfoMessage(buffer);
			}
		}
		break;
	case IDM_IMPORTTWXV2:
		memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
		of.lStructSize = OPENFILENAME_SIZE_VERSION_400;
#else
...
#endif
		of.hwndOwner = hwnd;
		of.lpstrFilter = "TWX v2 (XML) Files\0*.TWX;*.XML\0All Files\0*\0\0\0";
		of.lpstrCustomFilter = NULL;
		of.nFilterIndex = 1;
		strcpy (filename, "");
		of.lpstrFile = filename;
		...
		of.lpstrFileTitle = NULL;
		of.lpstrInitialDir = NULL;
		of.lpstrTitle = "Import TWX v2 File";
		of.Flags = 0;
		if (GetOpenFileName(&of)) 
		{
			...
			tmpPtr = import_twxv2(tmpFileName, XML_OFFSET);
			...
			if (strcmp (tmpPtr, " ") ==1)
			{
				...
				attacInfoMessage(tmpPtr);
			}
			else
			{
				sprintf (buffer2, "ErrorCode: %d -- Error importing %s", GetLastError(), tmpFileName);
				attacErrorMessage(buffer2);
			}
			pfree(&tmpPtr);
		}
		break;
	case IDM_IMPORTTXTCIM:
		memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
		of.lStructSize = OPENFILENAME_SIZE_VERSION_400;
#else
		...
		of.lpstrFileTitle = NULL;
		of.lpstrInitialDir = NULL;
		of.lpstrTitle = "Import CIM File in Text Format";
		of.Flags = 0;
		if (GetSaveFileName(&of)) 
		{
			returnVal = parseCIMFromTextFile(filename);
			...
			if (returnVal==1)
			{
				...
				sprintf (buffer, "CIM file %s imported", filename);
				attacInfoMessage(buffer);
			}
		}
		break;
	case IDM_EXPORTTWX:
		...
		if (GetSaveFileName(&of)) 
		{
			...
			returnVal = export_twx(tmpFileName);
			...
			if (returnVal == 0)	{
				...
				sprintf (buffer, "TWX file %s exported", tmpFileName);
				attacInfoMessage(buffer);
			} else {
				sprintf (buffer, "Error Code: %d -- Error exporting TWX file %s", GetLastError(), tmpFileName);
				attacErrorMessage(buffer);
			}
		}
		break;
	case IDM_EXPORTTWXV2:
		memset(&of, 0, sizeof(of));
#ifdef OPENFILENAME_SIZE_VERSION_400
		of.lStructSize = OPENFILENAME_SIZE_VERSION_400;
#else
		...
		if (GetSaveFileName(&of)) 
		{
			...
			//make sure to include all times
			returnVal = export_twxv2(tmpFileName, 0, ATTAC_Time()+100000, 0);
			//if it was a good import
			if (returnVal == 0)	{
				sprintf (buffer, "TWX v2 file %s exported", tmpFileName);
				attacInfoMessage(buffer);
			} else {
				sprintf (buffer, "Error Code: %d -- Error exporting TWX v2 file %s", GetLastError(), tmpFileName);
				attacErrorMessage(buffer);
			}
		}
		break;
	case IDM_EXPORTALLCIMDATA:
		exportAllCIMData_Text(1, 1);
		break;
	case IDM_EXPORTALLSECTORDATA:
		exportCIMSectorData_Text();
		break;
	case IDM_EXPORTALLPORTDATA:
		exportCIMPortData_Text();
		break;
	case IDM_REMOTECONNECTION:
		showremoteconnectionform(hwnd);
		break;
	case IDM_TWXSYNC:
		showtwxsyncform(hwnd);
		break;
	case IDM_TRADEWARS_NOTES:
		strcpy (fullFilePath, appDir);
		strcat (fullFilePath, "tradewarsNotes.txt");
		ShellExecute (hwnd, "open", "notepad.exe", fullFilePath, NULL, SW_SHOWNORMAL);
		break;
	case IDM_DEBUGINFO:
		printdebuginfo();
		break;
	case IDM_SWITCHDATABASE:
		showswitchdatabaseform(hwnd);
		break;
	case IDM_SETALLIES:
		showsetallies(hwnd);
		break;
	case IDM_LIMPITSCANNER:
		showlimpitscannerform(hwnd);
		break;
	case IDM_SYSOPPORTAMOUNT:
		startSysopPortScript();
		break;
	case IDM_SYSOPADDTOCLOSEDGAME:
		startSysopAddToClosedGame();
		break;
	case IDM_RECORDFASTMACRO:
		/* here we update the menu for the checkboxes */
		hmenu = GetMenu(hwnd);

		if (recordFastMacro>=0)
		{
			recordFastMacro=-1;
			writeMacros();
			CheckMenuItem(hmenu, IDM_RECORDFASTMACRO, MF_UNCHECKED);
			sprintf (buffer2, "Fast Macro Recorded as: %s", fastMacroString);
			attacInfoMessage(buffer2);
		}
		else
		{
			recordFastMacro=0;
			fastMacroString[recordFastMacro]=0;
			CheckMenuItem(hmenu, IDM_RECORDFASTMACRO, MF_CHECKED);
			attacInfoMessage("Recording New Fast Macro...");
		}
		break;
	case IDM_PLAYFASTMACRO:
		attac_send(fastMacroString);
		break;
	case IDM_ATTACHOMEPAGE:
		break;
	case IDM_DOCUMENTATION:
		
		break;
	case IDM_REGISTER:
		showregistrationform(hwnd);
		break;
	case IDM_ABOUTATTAC:
		showabout(hwnd);
		break;
	  ...
	  case IDM_RECONF:
		...
		if (IsIconic(showmessagecenter)) {
			if (showmessagecenter)
				SetWindowText(showmessagecenter,
				  cfg.win_name_always ? message_center_name : icon_name);
		}
#endif
...
#endif
		of.hwndOwner = hwnd;
		of.lpstrFilter = "REXX Scripting Files\0*.REXX;*.ZRX\0All Files\0*\0\0\0";
		of.lpstrCustomFilter = NULL;
		of.nFilterIndex = 1;
		strcpy (filename, "");
		of.lpstrFile = filename;
...
#ifdef ATTAC_CODE
		if (showmessagecenter)
			SetWindowText(showmessagecenter, message_center_name);
#endif
	}
