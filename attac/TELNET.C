...
#ifdef ATTAC_CODE
static const struct Opt o_twxechomark = 
	{ DO, DONT, WILL, WONT, TWX_ECHOMARK, OPTINDEX_TWX_ECHOMARK, REQUESTED};

static const struct Opt *const opts[] = {
    &o_twxechomark, &o_naws, &o_tspeed, &o_ttype, &o_oenv, &o_nenv, &o_echo, 
...
#ifdef ATTAC_CODE
	if (o->ack == WILL && o->option == TWX_ECHOMARK)
		attacInfoMessage ("TWX Proxy recognized. Enabling TWX Proxy Integration with ATTAC");
#endif
...
static void do_telnet_read(Telnet telnet, char *buf, int len)
{
	...
	if (remoteConnectionEnabled)
	{
		for (i=0; i<TOTAL_CONNECTIONS; i++)
		{
			...
			{
				if (i==1 && remote==1 && debugMe==1)
					;
				else
				{
					if( send(theClients[i].sock, buf, len, 0) < 0)
	...
	switch (telnet->state) {
	  ...
	  case SEENIAC:
	    ...
	    else if (c == TWX_SCRIPT_LOADED) {
			attacInfoMessage("Loading a TWX Proxy Script");
			runningTWXScript=1;
			telnet->state = TOP_LEVEL;
	    }
	    else if (c == TWX_SCRIPT_TERMINATED) {
			attacInfoMessage("Terminating a TWX Proxy Script");
			runningTWXScript=0;
			telnet->state = TOP_LEVEL;
		}
#endif
