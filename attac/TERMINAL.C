...
void term_out(Terminal *term)
{
...
#ifdef ATTAC_CODE
		if (ZocWaitForSeq==1)
			logtraffic(term->logctx, (unsigned char) c, LGTYP_REXX);
#endif
	    /*
	...
	if ((c & -32) == 0 && term->termstate < DO_CTRLS) {
		...
		logtraffic(term->logctx, (unsigned char) c, LGTYP_ATTAC);
#endif
		break;
...
#ifdef ATTAC_CODE
	int getXCursor(Terminal *term) {
		return term->curs.x;
	}
	int getYCursor(Terminal *term) {
		return term->curs.y;
	}

	char * screen_xy(Terminal *term, int x, int y, int length)
	{
		int i, j;
		char screenBuffer[MAX_LENGTH], tBuff[3], *t;
		...
		if (i <term->rows)    
		{
			...
			idx = i * (term->cols + 1);
			dirty_run = dirty_line = (ldata[term->cols] != term->disptext[idx + term->cols]);
			term->disptext[idx + term->cols] = ldata[term->cols];

			for (j = x; j < term->cols && j <=length; j++, idx++) 
			{
				unsigned long tchar;
				unsigned long *d = ldata + j;
				scrpos.x = j;
				...
				tchar = (*d & (CHAR_MASK | CSET_MASK));
				sprintf (tBuff, "%c", (int)tchar);
				if (strlen(screenBuffer) < MAX_LENGTH)
					strcat (screenBuffer, tBuff);
			}
		}
		t = (char *) strdup (screenBuffer);
		return (t);
	}
#endif
