...
#include "attac_src/util.h"
#include "attac_src/win_res.h"
#include "attac_src/writeMemory.h"
#include "attac_src/structs.h"
				 ...
				 void *dlg, Config *cfg)
{
	...
	{
			MessageBox(globalHWnd, "The Database file was not found", "Game File Not Found", MB_OK | MB_ICONERROR);
	}

	cfg->universesize = header.sector_cnt;
	cfg->escapesector = header.escapeSector;
	cfg->terrasector = header.terra;
	cfg->daystoclearbusts = header.daysToClearBust;

	cfg->cimsectors = header.cimUpdateSectorsDefault;
	cfg->cimports = header.cimUpdatePortsDefault;
	
	
	cfg->fighters_action_toll = header.tollTolls;
	cfg->fighters_action_def = header.defensiveTolls;
	cfg->blindtwarpprotection = header.blindTwarpProtection;
	cfg->selfdestructprotection = header.selfDestructProtection;

	cfg->remoteconnectionport = header.remoteConnectionPort;

	cfg->stealfactor = header.stealFactor;

	cfg->worsttradepercent= header.worstFactor;

	strcpy (cfg->fedcommencrypt, header.fedCommKey);
	strcpy (cfg->validstardockkeys, header.validStardockKeys);

	cfg->updateinfoonstartup = header.updateInfoBarStartup;
	cfg->sendannouncementstartup = header.announcementStartup;
	cfg->makewindowsdetachable = header.detachWindows;
	cfg->usepopupwindowsforcomm = header.popupComms;

	cfg->autohaggle = header.autoHaggle;
	cfg->updatemapwhenmoving = header.autoUpdateMap;
	
	strcpy (cfg->remoteconnectionpassword, header.remoteConnectionPassword);

	cfg->minpairportpercent = header.minPairPortPercent;

	strcpy (cfg->twxsyncserver, header.TWXSyncServer);
	cfg->twxsyncport = header.TWXSyncPort;

	cfg->popupmessagewhenrexxcomplete = header.popupOnRexxCompletion;

	clearBustsFunc();

	writeSetReady();

	SetTimer(globalHWnd, 2, 30000, writeMemoryTimerProc);
#endif
				 ...
				 void *data, int event)
{
	...
	if (ctrl == ssd->listbox || ctrl == ssd->loadbutton) {
		...
		{
			FILE *fp;
			//clicked save. update the attac header data
			//data is saved below
			updateATTACSettings(cfg);
			strcpy (gameName, savedsession);
...
#ifdef ATTAC_CODE
		{
			...
			char buffer2[256];
	//delete the file
			sprintf (buffer2, "Do you really want to delete the game '%s'", ssd->sesslist->sessions[i]);
			retVal = MessageBox (globalHWnd, buffer2, "Confirm Delete", MB_OKCANCEL);
			if (retVal == IDOK)
			{
				char fullFilePathBackup[MAX_LENGTH];

				strcpy (gameName, ssd->sesslist->sessions[i]);
				...
				strcpy (fullFilePathBackup, fullFilePath);
				strcat (fullFilePathBackup, ".bak");

				retVal = _unlink (fullFilePath);

				if (retVal!=0)
				{
//				  sprintf (buffer, "Error Deleting game %s", name);
//				  attacErrorMessage(buffer);
				}
				retVal = _unlink (fullFilePathBackup);
				if (retVal!=0)
				{
//				  sprintf (buffer, "Error Deleting game %s", name);
//				  attacErrorMessage(buffer);
				...
				get_sesslist(ssd->sesslist, TRUE);
				dlg_refresh(NULL, dlg);
				dlg_listbox_select(ssd->listbox, dlg, 0);
			}
		}
#else
		del_settings(ssd->sesslist->sessions[i]);
...
#ifdef ATTAC_CODE
			//check to make sure that they user has a proper ATTAC database
		{
			FILE *fp;
			char buffer3[MAX_LENGTH];
			char buffer4[MAX_LENGTH];

			strcpy (buffer4, "You must properly create a game before you can use ATTAC. Otherwise, ATTAC might crash. The steps are detailed below to properly create a game\n\n");
			strcat (buffer4, "* Start ATTAC, the ATTAC Configuration Screen should come up.\n");
			strcat (buffer4, "* Then type in a hostname (the telnet address) and the Port on the same screen and also type in a name for the Saved Session. \n");
			strcat (buffer4, "* THEN using the treeview on the left. select ATTAC Settings. Change the universe size here AS WELL AS any other options. \n");
			strcat (buffer4, "* THEN using the treeview on the left. select Miscellaneous. Here are more settings that you can change. \n");
			strcat (buffer4, "* THEN using the treeview on the left. select Session. This brings you back to the original screen. Click Save. Then your game is now saved!!! \n");
			strcat (buffer4, "* You can then click Open to start ATTAC.\n\n");
			strcat (buffer4, "The next time you start ATTAC, you can then either double click on the Saved Session OR you can click once on the saved session, click Load then click Open. \n");
			strcat (buffer4, "The two most important steps are to make sure the universe size is correct and that you Save the Settings at the end. ATTAC will run almost flawlessly once you do this. \n\n");
			strcat (buffer4, "Delete this game and the follow the steps above to recreate it properly!\n");

			if (strcmp (savedsession, "")==0)	//no saved session
			{
				MessageBox(globalHWnd, buffer4, "ATTAC Error", MB_OK | MB_ICONERROR);
				MessageBeep(0);
				return;
			}


			strcpy (buffer3, appDir);
			strcat (buffer3, savedsession);

			if (strstr(buffer3, ".atc")==NULL)
				strcat (buffer3, ".atc");
		...
		if (promptSave==1)
		{
			if (strcmp(savedsession, "Default Settings") != 0 && strcmp(savedsession, "") != 0)
			{
				int retVal;
				char buffer2[256];
				//we want to prompt to save
				sprintf (buffer2, "You made changes to game '%s', do you wish to save those changes?", savedsession);
				retVal = MessageBox (globalHWnd, buffer2, "Save Game", MB_YESNO);
				if (retVal == IDYES)
				{
					FILE *fp2;
					char tmpBuf[100];
					...
					header.daysToClearBust = cfg->daystoclearbusts;

					header.cimUpdateSectorsDefault = cfg->cimsectors;
					header.cimUpdatePortsDefault = cfg->cimports;

					header.tollTolls = cfg->fighters_action_toll;
					header.defensiveTolls = cfg->fighters_action_def;
					header.blindTwarpProtection = cfg->blindtwarpprotection;
					header.selfDestructProtection = cfg->selfdestructprotection;

					header.stealFactor = cfg->stealfactor;

					header.worstFactor = cfg->worsttradepercent;

					strcpy (header.fedCommKey, cfg->fedcommencrypt);
					strcpy (header.validStardockKeys, cfg->validstardockkeys);

					header.updateInfoBarStartup = cfg->updateinfoonstartup;
					header.announcementStartup = cfg->sendannouncementstartup;
					header.detachWindows = cfg->makewindowsdetachable;
					header.popupComms = cfg->usepopupwindowsforcomm;

					header.autoHaggle = cfg->autohaggle;
					header.autoUpdateMap = cfg->updatemapwhenmoving;
					

					header.remoteConnectionPort = cfg->remoteconnectionport;
					strcpy (header.remoteConnectionPassword, cfg->remoteconnectionpassword);
					header.minPairPortPercent = cfg->minpairportpercent;

					strcpy (header.TWXSyncServer, cfg->twxsyncserver);
					header.TWXSyncPort = cfg->twxsyncport;
					...
					if (fp2==NULL)
					{
						sprintf (tmpBuf, "Game file with universe size of %ld was just created", cfg->universesize);
						MessageBox(globalHWnd, tmpBuf, "Information", MB_OK);
					}
					else
						fclose (fp2);
		      ...
		      int midsession, int protocol)
{
    ...
    ctrl_settitle(b, "ATTAC", "Options controlling the ATTAC Settings");

    s = ctrl_getset(b, "ATTAC", "attac", "Set various ATTAC options");

	if (!midsession)
	{
		c = ctrl_editbox(s, "Universe Size:", 'u', 50,
			 HELPCTX(no_help),
			 dlg_stdeditbox_handler, I(offsetof(Config,universesize)),
			 I(-1));

	}

	c = ctrl_editbox(s, "Days to Clear Busts:", 'd', 50,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,daystoclearbusts)),
		 I(-1));

	c = ctrl_editbox(s, "Escape Sector:", 'e', 50,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,escapesector)),
		 I(-1));

	c = ctrl_editbox(s, "Terra Sector:", 't', 50,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,terrasector)),
		 I(-1));

    s = ctrl_getset(b, "ATTAC", "fighters", "Fighters");

    ctrl_radiobuttons(s, "Tolls Fighters:", 'h', 1,
		      HELPCTX(no_help),
		      dlg_stdradiobutton_handler,
			  I(offsetof(Config,fighters_action_toll)),
		      "Pay Tolls", I(0),
		      "Kill Toll Fighters", I(1),
		      "Do Not Do Anything", I(2), NULL);

    ctrl_radiobuttons(s, "Defensive Fighters:", 'f', 1,
		      HELPCTX(no_help),
		      dlg_stdradiobutton_handler,I(offsetof(Config,fighters_action_def)),
		      "Kill Defensive Fighters", I(0),
		      "Do Not Do Anything", I(1), NULL);

    s = ctrl_getset(b, "ATTAC", "protection", "Protection");
    ctrl_checkbox(s, "Blind Transwarp Protection", 'r',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,blindtwarpprotection)));
    ctrl_checkbox(s, "Self Destruct Protection", 's',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,selfdestructprotection)));

    ctrl_settitle(b, "ATTAC/More Options",
		  "More ATTAC Options");

    s = ctrl_getset(b, "ATTAC/More Options", "moreoptions",
		    "Remote Connection Setup:");


	ctrl_editbox(s, "Remote Connection Port:", 'r', 50,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,remoteconnectionport)),
		 I(-1));

    ctrl_editbox(s, "Remote Connection Password:", 'e', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,remoteconnectionpassword)),
		 I(sizeof(((Config *)0)->remoteconnectionpassword)));


    s = ctrl_getset(b, "ATTAC/More Options", "attacportoptions", "Porting Options");
    ctrl_checkbox(s, "Auto Haggle", 'u',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,autohaggle)));

    ctrl_checkbox(s, "Update Map When Moving", 'h',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,updatemapwhenmoving)));

	ctrl_editbox(s, "Minimum Pair Port Percent:", 't', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,minpairportpercent)),
		 I(-1));


    s = ctrl_getset(b, "ATTAC/More Options", "attactwxsyncoptions", "TWXSync Options");
	
	ctrl_editbox(s, "Server:", 'z', 80,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,twxsyncserver)),
		 I(sizeof(((Config *)0)->twxsyncserver)));

	ctrl_editbox(s, "Port:", 'x', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,twxsyncport)),
		 I(-1));

    ctrl_settitle(b, "ATTAC/Miscellaneous Options",
		  "Miscellaneous ATTAC Options");

    s = ctrl_getset(b, "ATTAC/Miscellaneous Options", "stealfromport",
		    "Steal From Ports:");

	ctrl_editbox(s, "Steal Factor:", 't', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,stealfactor)),
		 I(-1));

	ctrl_editbox(s, "Worse Trade Percent:", 'w', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,worsttradepercent)),
		 I(-1));

    s = ctrl_getset(b, "ATTAC/Miscellaneous Options", "cimdefaults",
		    "CIM Defaults:");


    ctrl_checkbox(s, "Sectors", 's',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,cimsectors)));
    ctrl_checkbox(s, "Ports", 'p',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,cimports)));

    s = ctrl_getset(b, "ATTAC/Miscellaneous Options", "otheroptions",
		    "Other Options:");

    ctrl_editbox(s, "Communication Encryption:", 'r', 40,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,fedcommencrypt)),
		 I(sizeof(((Config *)0)->fedcommencrypt)));

    ctrl_editbox(s, "Valid Stardock Keys:", 'h', 48,
		 HELPCTX(no_help),
		 dlg_stdeditbox_handler, I(offsetof(Config,validstardockkeys)),
		 I(sizeof(((Config *)0)->validstardockkeys)));

    s = ctrl_getset(b, "ATTAC/Miscellaneous Options", "configoptions",
		    "Configuration Options:");

    ctrl_checkbox(s, "Update Information on Startup", '1',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,updateinfoonstartup)));
    ctrl_checkbox(s, "Send Announcement on Startup", '2',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,sendannouncementstartup)));
    ctrl_checkbox(s, "Make Message and Info Window Detachable", '3',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,makewindowsdetachable)));
    ctrl_checkbox(s, "Use Popup Window when Sending Communications", '4',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,usepopupwindowsforcomm)));
    ctrl_checkbox(s, "Popup Message Box When REXX Scripts Complete", '5',
		  HELPCTX(no_help),
		  dlg_stdcheckbox_handler, I(offsetof(Config,popupmessagewhenrexxcomplete)));
