...
void CTWXSyncIfc::NewConnection(long hNewTWXSync, const char *host, int port, const char *username, const char *helper)
{
	char buffer[50];
	// New connection
	...
	listview_insert(GetDlgItem(gDlg, IDC_LCONNECTIONS), 0, buffer, (char *) "Connecting...", Get_Time(), (char *) "n/a");
}
	...
	if (len == 0) {
		...
		if (hlist != NULL) {
			...
			if (indexId!=-1)	//delete item
			{
				...
				listview_insert_c(GetDlgItem(gDlg, IDC_LOLD), 0, host, user, time, helper);
			}
			else
				listview_insert(GetDlgItem(gDlg, IDC_LOLD), 0, buffer, "n/a", Get_Time(), "n/a");
...
void CTWXSyncIfc::NewConnectionCallback(long hTWXSync, long hNewTWXSync, const char *host, int port, const char *username, const char *helper)
{
	// New connection
	printf("\nNew connection: %s:%d\n", host, port);
	...
	if (validRegCode==1 || usersConnected==0) {
		...
		pTWXSyncIfc->NewConnection(hNewTWXSync, host, port, username, helper);
		usersConnected++;
	} else {
...
void CTWXSyncIfc::ValidUserLoginCallback(long hTWXSync, const char *host, int port, const char *username, long timeConnected, const char *helper)
{
	char buffer[128];
	sprintf (buffer, "%s:%d", host, port);
	//find item in connections
	...
	if (indexId!=-1)	//delete item
		listview_update_item (GetDlgItem(gDlg, IDC_LCONNECTIONS), indexId, (char *) host, port, (char *)username, timeConnected, (char *)helper);
}
